...
create view v_clients_accounts as select accounts.client_id, clients.first_name , clients.last_name , clients.email, accounts.account_no , accounts.balance  from accounts left join clients on clients.id = accounts.id

create v_accounts_types as select accounts.account_no ,accounts.client_id , account_types."name" from accounts left join account_types on account_types.id = accounts.id

create v_transactions_detail as select select transactions.id, transactions.amount , transactions_types."name"  from transactions left join transactions_types on transactions.id  = transactions_types.id 

create v_transations_types_detail as select select transactions.id, transactions.amount, transactions_types."name" from transactions_types left join transactions on transactions_types.id =   transactions.id 
